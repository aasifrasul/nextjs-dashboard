...
const create2DArray = (size: number, initialValue: any): any[][] => {
	return Array(size).fill(null).map(() => Array(size).fill(initialValue));
};
...
export class GamePlay {
	...
	toggleOrientation(): void {
		this.setupPhase.currentOrientation = 
			this.setupPhase.currentOrientation === Orientation.Horizontal ? Orientation.Vertical : Orientation.Horizontal;
		this.notifyListeners();
	}
	...
	canPlaceShip(player: Player, x: number, y: number, shipType: ShipType, orientation: Orientation): boolean {
	...
	placeShip(player: Player, x: number, y: number): PlaceShipResult {
		...
		if (allShipsPlaced) {
			...
			this.setupPhase.currentShipType = availableShips.length > 0 ? availableShips[0] : null;
		}
...
export const generateRandomNumberUnderNum = (num: number): number => Math.floor(Math.random() * num);